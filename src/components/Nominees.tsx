  ...
  const renderMedias = () => {
    if (medias) {
      if (medias.length > 0)
                ...
                >
                  <Media media={media}>
                    {/* <img
                                        ...
                                        /> */}
                    <img
                      src={media.img_src ? media.img_src : NoImageFound}
                      onError={(e: any) => {
                        e.target.src = NoImageFound; // some replacement image
                        // e.target.style = 'padding: 8px; margin: 16px' // inline styles in html format
                      }}
        ...
        return (
          <animated.h1 style={translateTitle} className="noResultText">
      ...
      {bannerAnimate((animation, item) => {
        return (
          ...
          item === MAX_NOMINEE && (
            ...
            >
              <h1 className="maxNomineesTitle">lIKED PICTURES</h1>
              <p className="maxNomineesDesc">You have picked your top 5</p>
              <div className="bannerBorder">
                <animated.span className="top" style={top}></animated.span>
                <animated.span className="right" style={right}></animated.span>
                <animated.span
                  className="bottom"
                  style={bottom}
                ></animated.span>
                <animated.span className="left" style={left}></animated.span>
              </div>
            </animated.div>
          )
        );
