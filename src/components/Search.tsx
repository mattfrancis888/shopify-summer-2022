...
const Searchbar: React.FC<SearchbarProps> = (props) => {
    ...
    async function fetchMyAPI() {
      ...
      axios
        ...
        .then((response) => {
          ...
          if (response.data.photos) dataSet(response.data.photos.slice(0, 5));
          else if (response.data.Error === MANY_ERROR) dataSet(MANY_ERROR);
  ...
  const renderSearchPreview = () => {
    if (showLoading) {
      ...
      return (
        <h1 className="noResultText">Enter a keyword in the search form</h1>
      );
    else if (data) {
      ...
      if (data === MANY_ERROR)
        return (
          <h1 className="noResultText">Too Many Results, Narrow Your Search</h1>
        );
